...
function Admin() {
  ...
  if (!token) {
    return <main className="admin-page"><section className="login-panel"><a className="admin-back" href="/"><ArrowLeft size={16} /> Portfolio</a><ShieldCheck size={33} /><p className="eyebrow">Private access</p><h1>Admin sign in</h1><p>Manage incoming portfolio messages.</p><form onSubmit={signIn}><label>Email<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} /></label><label>Password<input type="password" required value={password} onChange={(event) => setPassword(event.target.value)} /></label><button className="admin-button" disabled={loading}>{loading ? 'Signing in...' : 'Sign in'}</button>{error && <p className="admin-error">{error}</p>}</form></section></main>;
  }
